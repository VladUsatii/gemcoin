...
class PublicKey:
	...
	def toDer(self):
		hexadecimal = encodeConstructed(encodeConstructed(encodePrimitive(DerFieldType.object, _ecdsaPublicKeyOid), encodePrimitive(DerFieldType.object, self.curve.oid)), encodePrimitive(DerFieldType.bitString, self.toString(encoded=True)))
	...
	def fromDer(cls, string):
		...
		if publicKeyOid != _ecdsaPublicKeyOid:
			raise Exception("The Public Key Object Identifier (OID) should be {ecdsaPublicKeyOid}, but {actualOid} was found instead".format(ecdsaPublicKeyOid=_ecdsaPublicKeyOid, actualOid=publicKeyOid))
	...
	def fromString(cls, string, curve=secp256k1, validatePoint=True):
		...
		p = Point(x=intFromHex(xs), y=intFromHex(ys))

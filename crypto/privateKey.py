...
class PrivateKey:
	...
	def publicKey(self):
		curve = self.curve
		publicPoint = Math.multiply(p=curve.G, n=self.secret, N=curve.N, A=curve.A, P=curve.P)
	...
	def fromDer(cls, string):
		...
		if privateKeyFlag != 1:
			raise Exception("Private keys should start with a '1' flag, but a '{flag}' was found instead".format(flag=privateKeyFlag))
